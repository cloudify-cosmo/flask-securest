...
class RoleBasedAuthorizationProvider(AbstractAuthorizationProvider):
    ...
    def __init__(self, roles_config_file_name):
        config_file_path = os.path.join(os.environ[REST_SERVICE_HOME_ENV_VAR],
                                        roles_config_file_name)
        self.permissions_by_roles = \
            _load_permissions_from_file(config_file_path)

    def authorize(self):
        target_endpoint = rest_security.get_endpoint()
        target_method = rest_security.get_http_method()
        user_roles = current_app.securest_role_loader.get_roles()
    ...
    def _is_authorized(self, target_endpoint, target_method, user_roles):
        current_app.securest_logger.info(
            '***** starting is_authorized, target_endpoint: {0}, '
            'target_method: {1}, user_roles: {2}'.
            format(target_endpoint, target_method, user_roles))
...
def _is_endpoint_permitted(target_endpoint, permitted_endpoint):
    ...
    if re.match(pattern, target_endpoint):
        ...
        # requests to access "v2/blueprint"
        if permitted_endpoint.endswith('/*'):
