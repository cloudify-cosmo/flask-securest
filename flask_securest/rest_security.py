...
from flask_securest.role_loaders.abstract_role_loader import AbstractRoleLoader
...
class SecuREST(object):
    ...
    def __init__(self, app):
        ...
        self.app.securest_role_loader = None
    ...
    def set_role_loader(self, role_loader):
        """
        Registers the given role loader.
        :param role_loader: the role loader to be set
        """
        if not isinstance(role_loader, AbstractRoleLoader):
            err_msg = 'failed to register role loader "{0}", Error: ' \
                      'driver does not inherit "{1}"' \
                .format(utils.get_instance_class_fqn(role_loader),
                        utils.get_class_fqn(AbstractRoleLoader))
            _log(self.app.securest_logger, 'critical', err_msg)
            raise Exception(err_msg)

        self.app.securest_role_loader = role_loader
